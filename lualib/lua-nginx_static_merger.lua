...
if ffi.os == "Windows" then
    cache_root = "D:\\my\\openresty-demo\\www\\static\\cache"
    static_root = "D:\\my\\openresty-demo\\www\\static"
elseif ffi.os == "OSX" then
    cache_root = "/Users/jasonzhao/workspace/IdeaProjects/openresty-demo/www/static/cache"
    static_root = "/Users/jasonzhao/workspace/IdeaProjects/openresty-demo/www/static"
else
    cache_root = nil
    static_root = nil
end
...
M.generate_file = function(fpath)
    ...
    for key, value in ipairs(parts) do
        ...
        if fp1 ~= nil then
            ...
            ngx.log(ngx.ERR,"generate cache file:",err)
        end
    end
    -- save cache file
...
M.main = function()
    -- check cache path
    local fp,err = io.open(cache_root, "r")
    if fp == nil then
        if ffi.os == "Windows" then
            os.execute('md ' .. cache_root)
        else
            os.execute('mkdir ' .. cache_root)
        end

        M.log("Main() => cache path not exist => " .. cache_root, ngx.NOTICE)
        ngx.log(ngx.ERR,"create cache:",err)
    end
